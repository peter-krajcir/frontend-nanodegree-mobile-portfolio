...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    // second bottle-neck
    // all the pizza elements have the same offsetWidth, no reason to calculate the new value for each pizza
    // in the loop, let's just calculate it for one pizza and then re-apply to the rest of the pizzas
    // resize time cut from ~130ms to ~1.5ms
...
function updatePositions() {
  ...
  // first bottle-neck
  // function Math.sin in the loop with 200 items is pretty heavy operation
  // calculation moved outside ouf the loop and it's calculated 5 times (number of pizzas per row)
  // instead of 200 per scroll
  var phases = [];
  phases[0] = Math.sin((document.body.scrollTop / 1250) );
  phases[1] = Math.sin((document.body.scrollTop / 1250) + 1);
  phases[2] = Math.sin((document.body.scrollTop / 1250) + 2);
  phases[3] = Math.sin((document.body.scrollTop / 1250) + 3);
  phases[4] = Math.sin((document.body.scrollTop / 1250) + 4);
